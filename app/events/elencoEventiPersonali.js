...
router.get("/:data", async (req, res) => {
    ...
    var file_pub = req.header('file-name-pub');
    var file_per = req.header('file-name-per');
    ...
    if(eventsPers.length > 0 || eventsPub.length > 0) {
        eventsPers = eventsMap.map(eventsPers, file_per, token);
        eventsPub = eventsMap.map(eventsPub, file_pub, token);
